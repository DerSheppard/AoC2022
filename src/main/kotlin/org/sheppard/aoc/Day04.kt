...
class Day04(override val raw: String) : Day {
    ...
    override fun part1(): String {
        return getSectors().map { it[0] >= it[2] && it[1] <= it[3] || it[2] >= it[0] && it[3] <= it[1] }.count { it }.toString()
    }
    ...
    private fun getSectors(): List<List<Int>> = lines.map { it.split('-', ',').map(String::toInt) }
}